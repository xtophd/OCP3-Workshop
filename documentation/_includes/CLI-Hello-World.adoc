...
Normally files served by httpd go into /var/www/html, but the security-conscious random uid does not have permissions to write to this directory (or any other directory than the tmp dirs).
...
To save time and avoid the complexity of editing an HTML file, we will just copy an existing file into the running container.
...
There is a more thorough explanation of storage in the WebUI portion of this workshop, so for now let's avoid technical talk and just go through the motion.
...
CAUTION: Do NOT actually do the next *Clean Up (Informational Only)* step during the WORKSHOP Today!

== Clean Up (Informational Only)

WARNING: We utilize the deployed applications from these exercises for the unit covering the WebUI.  We provide the commands needed for general cleanup as information only.

CAUTION: Reminder: Do NOT actually complete this "Clean Up" step during the WORKSHOP Today. You will reuse component for later exercises.
